window.__ModuleLoader__.load({
	id: "@dsh-pro/client-ui-layout-wide",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		const tagId = "@dsh-pro/client-ui-layout-wide/AppFrame.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@dsh-pro/client-ui-layout-wide";
		...
		function createLayoutStore() {
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					openDetails: (d) => {
						// 0.40, not the stock 0.35: the canvas renders real documents and
						// pages, and a third of a window is not enough to read one in.
						// The clamps are unchanged, so a drag still overrides this.
						if (d.details === 0) d.details = Math.max(420, Math.round(d.viewport * 0.4));
					},
