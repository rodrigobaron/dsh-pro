/**
 * at-file client plugin: the browser half of the Codex-style @file
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, { en, zh: en }), 'at-file: dictionaries')
  ...
  const reportSettingsError = (
    ...
    if (typeof error === 'object' && error !== null && 'code' in error && 'message' in error) {
      ...
      console.error(`[at-file] settings ${operation} failed: ${remoteError.code}: ${remoteError.message}`)
      return
    }
    console.error(`[at-file] settings ${operation} failed:`, error)
  }
  ...
  ctx.effect(async () => {
    ...
    if (atFile === undefined) {
      throw new Error('at-file: the atFile Remote namespace did not mount')
    }
    await loadSettings()
    return () => {
  ...
  const search = async (sessionId: SessionId, signal: AbortSignal): Promise<readonly FileEntry[]> => {
    if (atFile === undefined) throw new Error('at-file: the atFile Remote is not mounted')
  ...
  const openPath = (path: string): void => {
    void connection.api.host.openPath({ path }).then((response: OpenPathResponse) => {
      if (!response.result.ok) console.error('[at-file] open failed:', response.result.error.message)
    }, (error: unknown) => {
      console.error('[at-file] open failed:', error)
    })
  }
  ...
  const openRelative = (relative: string): void => {
    ...
    if (entry === undefined) {
      console.error('[at-file] open failed: no index entry for', relative)
      return
    }
    openPath(entry.path)
  }
  ...
  ctx.slots.inject('conversation.input.overlay', () => ctx.slots.register({
    ...
    inject: (sessionId): FolderNavigatorInjected => {
      ...
      if (actx === undefined) throw new Error(`at-file: session "${String(sessionId)}" has no client scope`)
