/**
 * at-file host plugin: mounts the `atFile` Typert Remote service
...
/** Cordis plugin name. The client bundle id is the installed package name,
 * set by the build's ModuleLoader banner, not by this. */
...
export function apply(ctx: Context, config?: Config): void {
  ...
  ctx.on('agent/created', ({ agent }) => {
    agent.ctx.effect(() => {
      ...
      return () => { stop() }
    }, 'at-file: pre-step path references')
  })
  /* v8 ignore stop */
}
