 ...
 *
 * This plugin imports harness packages for their types and for the module
 * augmentations that declare the settings slot it fills and the session
 * projection seam it registers into. The packages are runtime-provided and the
 * build externalizes them, so they are not dependencies; they are only needed
 * to type-check.
 *
...
async function findHarnessScope() {
  ...
  if (existsSync(join(fromProfile, 'dsh-session-projection'))) return fromProfile

  const npxRoot = join(homedir(), '.npm', '_npx')
  if (existsSync(npxRoot)) {
    for (const entry of await readdir(npxRoot)) {
      const candidate = join(npxRoot, entry, 'node_modules', '@deepseek-ai')
      if (existsSync(join(candidate, 'dsh-session-projection'))) return candidate
    }
  }
